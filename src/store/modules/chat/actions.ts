...
const actions: ActionTree<ChatState, RootState> = {
  async addGroup({ commit, state, dispatch, rootState }, res : ServerRes){
   ...
   joinGroup({ commit, state, dispatch, rootState }, res : ServerRes){ 
    ...
    console.log('test1', user.userId, rootState.app.user.userId)
  ...
  serviceMessage( { commit, state, dispatch, rootState }, message: String){
    dispatch('groupMessage', {code: RCode.OK, msg:'', data: {
      userId: '0',
